...
filenames = sorted(glob.glob("/Users/aravindsudharsan/Desktop/UCSB/Spring 2018/Introduction to Deep Learning/Project/101_ObjectCategories/*/*.jpg"))
batch_size = 1
num_epochs = 50
...
def read_my_file_format(filename_queue, randomize=False):
    ...
    uint8image = tf.image.resize_images(uint8image, [210, 210]) #224 original
    if randomize:
...
def colornet(_tensors):
    ...
    with tf.variable_scope('colornet'):
        ...
        conv1 = tf.image.resize_bilinear(conv1, (53, 53))
        ...
        conv2 = tf.image.resize_bilinear(conv2, (105, 105))
        ...
        conv3 = tf.image.resize_bilinear(conv3, (210, 210))
        ...
        #conv5_1 = conv2d(conv5, _tensors["weights"][
                       #'wc5_1'], sigmoid=False, bn=True)
...
with tf.variable_scope('colornet'):
    ...
    weights = {
        ...
        #'wc5_1': tf.Variable(tf.truncated_normal([3, 3, 3, 3], stddev=0.01)),
...
if uv == 1:
    loss = tf.split(loss,3, 2,)[0]
elif uv == 2:
    loss = tf.split(loss, 3, 2)[1]
else:
    loss = (tf.split(loss,3,2)[0] + tf.split(loss,3,2)[1]) / 2
...
try:
    while not coord.should_stop():
        ...
        if step % 1 == 0:
            ...
            writer.flush()
        if step  == 304800:
            save_path = saver.save(sess, "/data/home/aravindsudharsan/colornet/model.ckpt")
